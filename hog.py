...
def free_bacon(score):
    ...
    i = 0
    while score > 0:
        digit[i], score = score % 10, score // 10
        i += 1
    return max(digit) + 1
    # END PROBLEM 2
...
def is_swap(score0, score1):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    small = min(score0, score1)
    big = max(score0, score1)
    if small <= 1:
        return False
    if big % small == 0:
        return True
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score[0] < goal and score[1] < goal:
        cur_strtgy = strategy[player]
        score[player] += take_turn(cur_strtgy(score[player], score[other(player)]), score[other(player)], dice)
        if is_swap(*score):
...
def both(f, g):
    ...
    "*** YOUR CODE HERE ***"
    def say(score0, score1):
        f_plus = f(score0, score1)
        g_plus = g(score0, score1)
        return both(f_plus, g_plus)
    return say
    # END PROBLEM 6
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def amazing_gain(*score):
        current_high = score[who] - previous_score
        if current_high > previous_high:
            if current_high == 1:
                tail = 'point!'
            else:
                tail = 'points!'
            print(current_high, tail, "That's the biggest gain yet for Player", who)
        else:
            current_high = previous_high
        return announce_highest(who, current_high, score[who])
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    best_roll, best_average = 0, 0
    for roll_num in range(10, 0, -1):
        temp = averaged(roll_num, dice)
        if best_average <= temp:
            best_roll = roll_num
            best_average = temp
    return best_roll
    # END PROBLEM 9
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    # BEGIN PROBLEM 10
    if opponent_score != 0:
        lower, higher = opponent_score % 10, opponent_score // 10
        if lower >= margin-1 or higher >= margin-1:
            num_rolls = 0
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if opponent_score != 0:
        ...
        swap = is_swap(score + max(lower, higher) + 1, opponent_score)
        if not swap and bacon_roll == 0 :
            num_rolls = 0
